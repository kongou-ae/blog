...
{{< figure src="/images/2019-09-07-002.png" title="ジオ" >}}
...
{{< figure src="/images/2019-09-07-003.png" title="リージョン" >}}
...
{{< figure src="/images/2019-09-07-004.png" title="ゾーン" >}}
...
{{< figure src="/images/2019-09-07-005.png" title="スケールユニット" >}}
...
{{< figure src="/images/2019-09-07-001.png" title="更新ドメイン" >}}

> 所定の可用性セットに対して、同時に再起動される仮想マシンのグループと物理ハードウェアを示す、ユーザーが構成できない 5 つの更新ドメインが既定で割り当てられます

https://docs.microsoft.com/ja-jp/azure/virtual-machines/windows/manage-availability#configure-multiple-virtual-machines-in-an-availability-set-for-redundancy

冗長化している仮想マシンが同一の更新ドメイン上で動作している場合、2台仲良くメンテナンスで再起動する可能性があります。
...
{{< figure src="/images/2019-09-07-007.png" title="可用性セット" >}}
...
{{< figure src="/images/2019-09-07-008.png" title="可用性ゾーン" >}}

複数の仮想マシンが動作するゾーンを分散すれば、1つのスケールユニットが停止したとしても他のゾーンのスケールユニットで動作する仮想マシンは動き続けます。また、1つのデータセンタ内のすべてのスケールユニットが停止したとしても、他のゾーンのスケールユニットで動作する仮想マシンは動き続けます。
...
利用者は仮想マシンを作成する際にリージョンを明示的に指定する必要があります。複数の仮想マシンの動作するリージョンを分散すれば、1つのリージョンが停止したとしても他のリージョンで動作する仮想マシンは動き続けます。

{{< figure src="/images/2019-09-07-009.png" title="リージョン指定" >}}
...
ローカル冗長ストレージ （LRS）は、特定のストレージスケールユニットの中の3箇所にデータを複製します。したがって、可用性セットの恩恵によって、ラック単位の障害や Microsoft のメンテナンスによる影響を回避できます。

{{< figure src="/images/2019-09-07-010.png" title="LRSのイメージ" >}}
...
{{< figure src="/images/2019-09-07-011.png" title="ZRSのイメージ" >}}
...
ゾーンにまたがってデータが複製されているので、可用性ゾーンの恩恵によってデータセンタ単位の障害を回避できます。一方で、リージョン全体が影響を受けるような障害や災害が発生した場合、ZRS のストレージに保存されているデータが消失する可能性があります。

### ジオ冗長ストレージ (GRS)

ZRS のリスクを回避する手段の１つがジオ冗長ストレージ （GRS）です。GRS は LRS のデータを非同期でペアリージョンのストレージスケールユニットに3つ複製します。ペアリージョン側も LRS です。したがって複製の合計は6つです。

{{< figure src="/images/2019-09-07-013.png" title="GRSのイメージ" >}}
...
ただし、安心のGRSにも次の注意点があります。ただし、本エントリの趣旨から外れるため詳細を割愛します。
...
GRS には弱点があります。それはメインリージョン側の可用性が LRS であるという点です。もし、１つのスケールユニットが停止した場合、メインリージョン側に保存されているデータにアクセスできなくなります。スケールユニット内のデータが消失した場合、上述のとおりマイクロソフトがリージョンの切り替えを判断するまでペアリージョンのデータを読み書きできません。

この弱点を解消するのが GZRS です。GZRS ではメインリージョン側が ZRS になっています。そのため、単一のストレージスケールユニットやデータセンターで障害が起きても、データにアクセスし続けられます。Microsoft によるリージョンの切り替えを待つ状況が発生しにくくなっています。

{{< figure src="/images/2019-09-07-014.png" title="GZRSのイメージ" >}}
